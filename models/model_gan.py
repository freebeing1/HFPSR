...
class ModelGAN(ModelBase):
    ...
    def load(self):
        ...
        if self.opt['is_train'] and load_path_D is not None:
            ...
            self.load_network(load_path_D, self.netD,
                              strict=self.opt_train['D_param_strict'])
    ...
    def define_loss(self):
        ...
        if self.opt_train['F_lossfn_weight'] > 0:
            ...
            if self.opt['dist']:
                self.F_lossfn = PerceptualLoss(feature_layer=F_feature_layer, weights=F_weights, lossfn_type=F_lossfn_type,
                                               use_input_norm=F_use_input_norm, use_range_norm=F_use_range_norm).to(self.device)
            else:
                self.F_lossfn = PerceptualLoss(feature_layer=F_feature_layer, weights=F_weights,
                                               lossfn_type=F_lossfn_type, use_input_norm=F_use_input_norm, use_range_norm=F_use_range_norm)
    ...
    def define_optimizer(self):
        ...
        ), lr=self.opt_train['D_optimizer_lr'], weight_decay=0)
    ...
    def optimize_parameters(self, current_step):
        ...
        if self.opt_train['gan_type'] in ['gan', 'lsgan', 'wgan', 'softplusgan']:
            ...
            # real
            # 1) fake data, detach to avoid BP to G
