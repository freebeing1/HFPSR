...
from models.loss import HighFrequencyPreservingLoss
...
class ModelPlain(ModelBase):
    ...
    def __init__(self, opt):
        ...
        self.high_pass_filter = get_high_pass_filter(opt).to(self.device)
    ...
    def define_loss(self):
        ...
        elif G_lossfn_type == 'hfpl':
            self.G_lossfn = HighFrequencyPreservingLoss(
                self.opt_train['G_hfpl_params'], self.high_pass_filter).to(self.device)
        else:
    ...
    def feed_data(self, data, need_H=True):
        ...
        if need_H:
            self.H = data['H'].to(self.device)  # 0 ~ 1 (float32)
    ...
    def optimize_parameters(self, current_step):
        ...
        if isinstance(net_loss, tuple):
            net_loss, sub_loss = net_loss[0], net_loss[1]

        net_loss = self.G_lossfn_weight * net_loss
        net_loss.backward()
        ...
        self.log_dict['net_loss'] = net_loss.item()
        if self.opt_train['G_lossfn_type'] == 'hfpl':
            for loss_name, _ in self.opt_train['G_hfpl_params'].items():
                self.log_dict[loss_name] = sub_loss[loss_name]
    ...
    # get L, E, H image
    # ----------------------------------------
    def current_visuals(self, need_H=True, need_HF=False):
        ...
        out_dict['E'] = self.E_SB.detach()[0].float().cpu()
