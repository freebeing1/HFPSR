...
# --------------------------------------------
# High Frequency Preserving Loss (HFPL)
# --------------------------------------------


class HighFrequencyPreservingLoss(nn.Module):
    def __init__(self, opt, hpf):
        ...
        self.pixel_loss_hf = nn.L1Loss()
        self.hb_loss = nn.L1Loss()

    def forward(self, x, gt):
        """
        Args:
            x (torch.Tensor): Model output tensor with shape (2, b, c, h, w)
            gt (torch.Tensor): Ground truth tensor with shape (b, c, h, w)
        Return:
            net_loss (torch.Tensor): the weighted sum of 4 losses
                                     1. "perceptual": perceptual loss
                                     2. "pixel": pixel loss of SR branch (L1)
                                     3. "pixel_hf": pixel loss of high frequency component of SR branch (L1)
                                     4. "pixel_hb": pixel loss of HF branch (L1)
            sub_loss (dict): dictionary of each loss with no weight
        """
        x_sb, x_hb = x[0], x[1]
        ...
        net_loss += self.opt_lossfn['pixel'] * sub_loss['pixel']
        net_loss += self.opt_lossfn['pixel_hf'] * sub_loss['pixel_hf']
        net_loss += self.opt_lossfn['pixel_hb'] * sub_loss['pixel_hb']

        return net_loss, sub_loss
